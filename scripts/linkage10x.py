...
for site in sites:
	...
	for pileupColumn in pileup:
		for pileupRead in pileupColumn.pileups:
			if (pileupColumn.pos >= start) and (pileupColumn.pos < end) and (not pileupRead.is_refskip):
					...
					print ('%s\t%s\t%s\t%s\t%s\t%s\t%s' % (pileupRead.alignment.get_tag("RX"),pileupRead.alignment.query_name,site[0],pileupColumn.pos +1,refBase,base,site[3]))
bamfile.close()
